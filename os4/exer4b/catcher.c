...
char* send_end(pid_t sender_pid, int send_sig_num) {
    if (sender_pid == kill_sender_pid) {
        ...
        return "Sth went wrong in sending end from catcher\n";
    // not async safe but how else
    printf("Signals recieved in catcher:   %d\n\n", send_sig_num);
    exit(EXIT_SUCCESS);
}

char *send_confirm(pid_t sender_pid) {
    if (sender_pid == kill_sender_pid) {
        kill(sender_pid, SIG);
    } else if (sender_pid == sigq_sender_pid) {
        union sigval tmp;
        ...
        return "Sth went wrong in sending confirm from catcher\n";
    return "Signal catched in catcher\n";
}

void handler(int sig, siginfo_t *info, void *ucontext) {
    char *output;
    if (sig == SIG)
        output = send_confirm(info->si_pid);
    else if (sig == END_SIG) 
...
int main(int argc, char **argv) {
    ...
    // waiting for signals till SIGUSR2 then terminates in send_end
    while (true);
    return 0;
}