...
void pack() {
    ...
    if (semctl(sem_id, TO_PREP_I, GETVAL) <= 0) {
        update_sem(sem_id, CAN_MOD_I, 1); // we want to pack order so we need at least one order to pack
        return;
    }
    update_sem(sem_id, TO_PREP_I, -1);
    ...
    update_sem(sem_id, TO_SEND_I, 1); // we packed order so it is ready to send
